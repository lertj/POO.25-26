...
public:
    ...
    Produs& operator++() { //prefixat
        
        if (this->istoricPreturi != nullptr) {

            float lastPrice = this->istoricPreturi[this->nrPreturi - 1];

            float* copy = new float[this->nrPreturi + 1];
            for (int i = 0; i < this->nrPreturi; i++) {
                copy[i] = this->istoricPreturi[i];
            }
            ...
            this->istoricPreturi = copy;
            this->istoricPreturi[this->nrPreturi - 1] = lastPrice * 0.9;
    ...
    Produs& operator++(int) { //postfixat
        Produs* copy = new Produs;
        *copy = *this;
        if (this->istoricPreturi != nullptr) {
    ...
    // operatori care returneaza Clasa&/Clasa =,+=, -=, *=, +, -, *
    // operatori care returneaza void (nerecomandat)
    // operatori care returneaza bool ==, >, <, <=
    // misc. istream/ostream >>,<<
...
ostream& operator<<(ostream& out, const Produs& p) {
    ...
    out << "NrPreturi: " << p.nrPreturi << endl;
